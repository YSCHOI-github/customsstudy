## 2026-05-01 | 02_03_무역계약의성립

### 🔴 수정됨 (6건)
- [적용 규범 우선순위]: `CISG: ==영업소==가 서로 다른 체약국에 소재하는 당사자 간 물품매매계약에 적용됨` → `CISG: ==영업소==가 서로 다른 체약국에 소재하는 당사자 간 물품매매계약(제1조 제1항 a호) 또는 국제사법 규칙에 따라 체약국의 법률이 적용되는 경우(b호)에 적용됨` (근거: [[#CISG Article 1|CISG 제1조]] 제1항)
- [승낙 기간 기산일]: `전보 발송일 또는 서신 봉인일` → `전보는 발송을 위해 교부된 때, 서신은 서신에 표시된 일자(표시가 없는 경우 봉투의 소인일자)` (근거: [[#CISG Article 20|CISG 제20조]] 제1항)
- [청약의 효력 소멸 사유]: `6. 청약자의 사망 또는 행위능력 상실` → `6. 반대청약` (근거: [[#CISG Article 19|CISG 제19조]] 제1항)
- [도달의 의미]: `상대방에게 구두로 전달되거나 그의: 1. 영업소 2. 상거소 3. 우편주소` → `상대방 본인에게 직접 전달되거나 또는 그의: 1. 영업소 2. 우편주소 (없는 경우 상거소)` (근거: [[#CISG Article 24|CISG 제24조]])
- [적용 배제 매매]: `선박·수상운반구·공기선·부선 매매` → `선박·부선·수륙양용선 또는 항공기(Ships, vessels, hovercraft or aircraft) 매매` (근거: [[#CISG Article 2|CISG 제2조]] (e)호)
- [암기 속성 승낙기간]: `전보·서신: 발송일/봉인일` → `전보·서신: 발송을 위해 교부된 때 / 서신에 표시된 일자(또는 소인일자)` (근거: [[#CISG Article 20|CISG 제20조]] 제1항)

---

## 2026-05-01 | 02_03_무역계약의성립 (2차)

### 🔴 수정됨 (3건)
- [§1-1 ④ 적용 배제 매매]: `선박·수상운반구·공기선·부선(hovercraft)의 매매` → `선박·부선·수륙양용선(hovercraft) 또는 항공기(aircraft)의 매매` (근거: [[#CISG Article 2|CISG 제2조]] e호 — 항공기 누락 및 한글 표기 오류)
- [§1-1 ⑥ 소비자물품 매매 단서]: `단 매수인이 그러한 사용을 위해 구입한다고 알지 못했거나 알 수 없었을 경우는 적용됨` → `단, 매도인이 계약 체결 전이나 체결 시에 물품이 그러한 사용을 위한 것임을 알지 못했고 알 수도 없었던 경우에는 CISG가 적용됨` (근거: [[#CISG Article 2|CISG 제2조]] a호 — 주체는 매도인(seller))
- [§6-2 전자계약 성립 시기]: `발신주의: 전자문서를 수신자의 컴퓨터 파일에 기록된 때` → `수신주의(도달주의): 전자문서가 수신자의 정보처리시스템에 입력된 때` (근거: 수신자 시스템 입력 기준은 도달주의이며 발신주의가 아님)

### 🟡 수정됨 (5건)
- [§2-1 확정성 보완 기준 대금 미명시]: 제14조 하위에 [[#CISG Article 55|CISG 제55조]] 근거 명시 추가 (근거: [[#CISG Article 55|CISG 제55조]])
- [§2-5 청약의 효력 소멸 사유]: 6개 항목을 모범답안 기준 5가지로 재구성 — 기간 경과 2개를 `시간의 경과([[#CISG Article 18|CISG 제18조]] 제2항)` 1개로 통합, 반대청약을 필수 5번째 항목으로 포함 (근거: 2020 37회 모범답안)
- [§4-2 도달의 의미]: `구두로 통고된 때(made orally to him)` 및 `직접 전달(delivered personally)` 누락 추가, 총 4가지 경로로 확장 (근거: [[#CISG Article 24|CISG 제24조]])
- [§5 정지/해제조건 영문 표기]: `Condition Precedent / Condition Subsequent` → `Suspensive Condition / Resolutory Condition` (근거: 기출 모범답안 공식 용어)
- [암기 속성 정리 ④ 도달의 의미]: 3가지 나열에서 구두통고·직접전달·영업소·우편주소 4가지로 확장, 상거소 보충성 명시 (근거: [[#CISG Article 24|CISG 제24조]])

### ✅ 이상 없음
- §0 적용 규범 우선순위, §1-2 영업소([[#CISG Article 10|CISG 제10조]]), §1-3 관습·관례([[#CISG Article 9|CISG 제9조]])
- §2-2 청약 효력발생(제15조), §2-3 청약 취소(제16조), §2-4 소멸(제17조)
- §3 승낙 전 섹션(CISG 제18~22조)
- §4-1 계약 성립 시기(제23조)
- 암기 속성 정리 ① ② ③ ⑤

---

## 2026-05-02 | 02_05_Incoterms2020

### 🔴 수정됨 (8건)
- [Introduction 7 다루지 않는 사항]: 기존 8가지에서 계약위반결과 및 불가항력을 분리하여 총 9가지 항목으로 정정
- [EXW 위험이전 시점]: "매수인이 수령한 때" → "매수인의 임의처분 상태로 놓인 때"로 정정
- [FCA 인도 장소 및 위험이전]: 인도/위험이전 서술을 "매도인 영업소(적재 시)"와 "기타 장소(처분에 놓인 때)"로 명확히 분리
- [CIF 복수운송인 위험부담 연장]: "적하보험/통선하증권 취득" → "매매계약서 내 명시적인 특약(합의) 기재"로 정정
- [CPT B9 매수인 부담비용]: "물품 수령 미이행" → 조문에 근거한 "B10 조항에 따른 통지의무 미이행"으로 정정
- [[[#CISG Article 9|CISG 제9조]] 제1항 용어]: "합의한 관례(practices) 와 확립된 관행(usages)" → "합의한 관행(usages)과 확립된 관례(practices)"로 정정
- [[[#CISG Article 70|CISG 제70조]] 효과]: 매도인 위반 시 "위험이전 불인정" → "매수인의 구제수단 행사에 영향을 미치지 않음(구제수단 보장)"으로 정정
- [Incoterms 물품 특정 요건]: 물품 특정에 대해 "묵시적 전제" → "명시적 조건 (B3 조항)"으로 정정
